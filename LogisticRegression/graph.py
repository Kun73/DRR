...
class ER_graph:
    ...
    def undirected(self):
        U = np.zeros((self.size, self.size))
        G = nx.erdos_renyi_graph(self.size, 0.8)
...
class RingPlus_graph:
    def __init__(self, number_of_nodes):
        self.size = number_of_nodes

    def undirected(self):
        U = Ring_graph(self.size).undirected()
        step = int(1)
        for i in range(self.size):
            for j in range(i, self.size, step):
                u = np.random.rand(1)
                if u[0] < 0.6:
                    U[i][j] = 1
        return U
