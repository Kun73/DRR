...
class LR_L4( object ):
    def __init__(self, n_agent, class1 = 0, class2 = 9, balanced = True, limited_labels = False ):
        self.class1 = class1
        self.class2 = class2
...
########################################################################################################################
    ...
    def localgrad(self, theta, idx, j = None ):  ## idx is the node index, j is local sample index
        if j == None:                 ## local full batch gradient
            ...
            grad_reg = self.reg * (theta[idx] / (1 + theta[idx] ** 2)**2)
    ...
    def grad(self, theta, idx = None): ## centralized stochastic/batch gradient
        if idx == None:                ## full batch
            if self.balanced == True:
                ...
                return np.sum(grad, axis = 0)/self.N + self.reg * (theta / (1 + theta ** 2)**2)
            if self.balanced == False:
            ...
            if self.balanced == True:
                ...
                grad_reg = self.reg * (theta / (1 + theta**2)**2)
                grad = grad_lr + grad_reg
                return grad
            if self.balanced == False:
